...
def _order_points(pts):
    ...
    while len(ordered) < len(pts):
        ...
        for idx in idxs:
            idx = int(idx)
            if not used[idx]:
                ordered.append(idx); used[idx] = True; moved = True; break
        if not moved:
            rem = np.where(~used)[0]
            if len(rem):
                ordered.append(int(rem[0])); used[rem[0]] = True
...
def load_csv(path):
    ...
    b_hp, a_hp = signal.butter(2, 0.3, 'high', fs=FS)
    for i in range(2):
        vel_xy[:, i] = signal.filtfilt(b_hp, a_hp, vel_xy[:, i]) if N > 9 else vel_xy[:, i]
    pos_xy = np.cumsum(vel_xy * dt, axis=0)
    for i in range(2):
        pos_xy[:, i] = signal.filtfilt(b_hp, a_hp, pos_xy[:, i]) if N > 9 else pos_xy[:, i]
...
def load_dataset():
    ...
    if missing: print(f'  Skipped {missing} CSV/PNG pairs (no image)')
    if skipped: print(f'  Skipped {skipped} samples (empty stroke target)')
...
def drift_augment(imu, rng):
    imu = imu.copy()
    n_segs = rng.integers(2, 6); seg_len = len(imu) // n_segs
    for s in range(n_segs):
        st = s*seg_len; en = min(st+seg_len, len(imu))
...
def augment_sample(imu, img_raw, img_target, rng):
    ...
    if rng.random() < AUG_DRIFT_PROB: imu = drift_augment(imu, rng)
    if rng.random() < AUG_DROP_PROB:  imu = dropout_augment(imu, rng)
    warp = rng.uniform(*AUG_WARP_RANGE)
    new_len = max(int(len(imu)*warp), 2)
    xs_o = np.linspace(0,1,len(imu)); xs_n = np.linspace(0,1,new_len)
    warped = np.stack([np.interp(xs_n, xs_o, imu[:,c]) for c in range(imu.shape[1])], 1).astype(np.float32)
    xs_out = np.linspace(0,1,N_TIMESTEPS)
    imu = np.stack([np.interp(xs_out, np.linspace(0,1,new_len), warped[:,c])
...
def augment_dataset(X_tr, yr_tr, yt_tr, seed=0):
    ...
    for k in range(AUGMENT_FACTOR):
        ...
        Xs.append(Xc); Yrs.append(Yrc); Yts.append(Ytc)
        print(f'  Augmentation pass {k+1}/{AUGMENT_FACTOR} done')
    return (np.concatenate(Xs,  0),
            np.concatenate(Yrs, 0),
            np.concatenate(Yts, 0))
...
def save_snapshot(model, X_snap, yr_snap, yt_snap, lbl_snap,
                  device, epoch, tr_loss):
    model.eval()
...
def main():
    ...
    print(f'  Loaded {len(X)} samples  '
          f'(target density: {y_target.mean():.4f})')
    ...
    tr_ds = TensorDataset(torch.from_numpy(X_sc).float(),
    ...
    if args.resume:
        if Path(model_pt).exists():
            ...
            print(f'  WARNING: --resume passed but no saved model found at {model_pt}; starting fresh')

    TARGET_LR = 3e-4; WARMUP = 10; PATIENCE = 50
    opt = torch.optim.AdamW(model.parameters(), lr=TARGET_LR/WARMUP, weight_decay=1e-4)
    sch = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        opt, T_0=10, T_mult=2, eta_min=1e-6)

    best_loss = float('inf'); pat_count = 0
    history   = {'loss': []}

    print(f'\nTraining for {EPOCHS} epochs...\n')
    for epoch in range(EPOCHS):
        if epoch < WARMUP:
            for pg in opt.param_groups: pg['lr'] = TARGET_LR*(epoch+1)/WARMUP

        model.train()
        tr_sum = tr_n = 0
        for x_b, yt_b in tr_ld:
            x_b  = x_b.to(device); yt_b = yt_b.to(device)
            opt.zero_grad()
            loss = image_loss(model(x_b), yt_b, pos_weight_t)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            opt.step()
            tr_sum += loss.item()*x_b.size(0); tr_n += x_b.size(0)
        tr_loss = tr_sum / tr_n
        history['loss'].append(tr_loss)

        if epoch >= WARMUP: sch.step(epoch-WARMUP)
        ...
        if (epoch+1) % SNAPSHOT_EVERY == 0 or epoch == 0:
            save_snapshot(model, X_snap, yr_snap, yt_snap, lbl_snap,
                          device, epoch, tr_loss)
    ...
    ax.set_title('BCE Loss (rendered stroke GT)'); ax.set_xlabel('Epoch')
    ax.legend(); ax.grid(True)
    plt.tight_layout()
    plt.savefig(MODELS_DIR / 'training_history_side_mount.png', dpi=150); plt.close()
    ...
    for pi in range(n_show):
        ...
        for c in range(3): axes[pi,c].set_xticks([]); axes[pi,c].set_yticks([])

    plt.tight_layout()
    plt.savefig(MODELS_DIR / 'predictions_side_mount.png', dpi=150); plt.close()
    print('Predictions saved.')

    best_ep = int(np.argmin(history['loss']))
    print(f'\nBest train BCE: {history["loss"][best_ep]:.4f} at epoch {best_ep+1}')
    print('='*60); print(f'Done. Model: {model_pt}')
